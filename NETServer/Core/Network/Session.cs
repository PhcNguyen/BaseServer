...
namespace NETServer.Core.Network
{
    ...
    internal class Session : ISession, IDisposable
    {
        ...
        private readonly SocketHandle _socketHandle;
        ...
        public Session(Socket socket)
        {
            ...
            _socket = socket;
            _socketHandle = new SocketHandle(socket, _id);
        ...
        public async Task Connect()
        {
            try
            {
                ...
                await _socketHandle.StartReceiveAsync();
            }
        ...
        public async ValueTask<bool> Send(object data)
        {
            if (!_socket.Connected) return false;
            switch (data)
            {
                case byte[] byteArray:
                    await _socketHandle.SendDataAsync(byteArray);
                    break;
                case string str:
                    await _socketHandle.SendDataAsync(ByteConverter.ToBytes(str));
                    break;
                default:
