    ...
    internal class Server
    {
        ...
        private async Task AcceptClientConnectionsAsync(CancellationToken token)
        {
            while (_isRunning == 1)
            {
                ...
                {
                    NPLog.Instance.Error<Server>($"Socket error: {ex.SocketErrorCode}, Message: {ex.Message}");
                }
                catch (Exception ex)
                {
                    NPLog.Instance.Error<Server>($"Unexpected error: {ex.Message}");
                }
            }
        }
        ...
        public void StopServer()
        {
            ...
            Task.Run(async () =>
            {
                ...
                {
                    NPLog.Instance.Error<Server>($"Error during disconnecting clients: {ex.Message}");
                }
                ...
                {
                    NPLog.Instance.Error<Server>($"Error during socket cleanup: {ex.Message}");
                }
            });

            NPLog.Instance.Info("Server stopped successfully.");
        }
