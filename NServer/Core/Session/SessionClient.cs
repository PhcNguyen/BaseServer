...
public class SessionClient(Socket socket, TimeSpan timeout) : ISessionClient
{
    ...
    public void Connect()
    {
        try
        {
            ...
            NLog.Instance.Info($"Session {_id} connected to ");
        }
        catch (Exception ex) when (ex is TimeoutException or IOException)
        {
            NLog.Instance.Error<SessionClient>($"Connection error for : {ex.Message}");
            Disconnect();
        }
        catch (Exception ex)
        {
            NLog.Instance.Error<SessionClient>($"Unexpected error for : {ex.Message}");
            Disconnect();
        }
    }
    ...
    public void Disconnect()
    {
        ...
        try
        {
            ...
            NLog.Instance.Info<SessionClient>($"Session {_id} disconnected from");
        }
        catch (Exception ex)
        {
