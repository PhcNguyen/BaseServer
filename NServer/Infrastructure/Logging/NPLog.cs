...
public class NPLog : NPLogBase
{
    ...
    public void DefaultInitialization()
    {
        ...
        Log(NPLogLevel.INFO, "Default initialization");
    }
    ...
    public bool ShouldLog(NPLogLevel level) => level >= DefaultLevel;
    ...
    public void Log(NPLogLevel level, string message)
    {
    ...
    /// <param name="level">Mức độ nhật ký.</param>
    /// <param name="message">Thông điệp nhật ký.</param>
    public void Log<TClass>(NPLogLevel level, string message) where TClass : class
    {
    ...
    /// </summary>
    public void Info(string message) => Log(NPLogLevel.INFO, message);

    public void Info<TClass>(string message) where TClass : class =>
        Log<TClass>(NPLogLevel.INFO, message);

    /// <summary>
    /// Ghi một thông điệp cảnh báo.
    /// </summary>
    public void Warning(string message) => Log(NPLogLevel.WARNING, message);

    public void Warning<TClass>(string message) where TClass : class =>
        Log<TClass>(NPLogLevel.WARNING, message);

    /// <summary>
    /// Ghi một thông điệp lỗi với một ngoại lệ.
    /// </summary>
    public void Error(Exception exception) =>
        Log(NPLogLevel.ERROR, NPLogFormatter.FormatExceptionMessage(exception));

    /// <summary>
    /// Ghi một thông điệp lỗi với một thông điệp và ngoại lệ.
    /// </summary>
    public void Error(string message, Exception exception) =>
        Log(NPLogLevel.ERROR, $"{message}: {NPLogFormatter.FormatExceptionMessage(exception)}");

    public void Error<TClass>(string message) where TClass : class =>
        Log<TClass>(NPLogLevel.ERROR, message);

    public void Error<TClass>(Exception exception) where TClass : class =>
        Log<TClass>(NPLogLevel.ERROR, NPLogFormatter.FormatExceptionMessage(exception));

    public void Error<TClass>(string message, Exception exception) where TClass : class =>
        Log<TClass>(NPLogLevel.ERROR, $"{message}: {NPLogFormatter.FormatExceptionMessage(exception)}");
}