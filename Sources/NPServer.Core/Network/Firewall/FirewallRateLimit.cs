...
/// </summary>
public sealed class FirewallRateLimit(int maxAllowedRequests, int timeWindowInMilliseconds, int lockoutDurationInSeconds)
    : IFirewallRateLimit
{
    ...
    public bool IsAllowed(string ipAddress)
    {
        ...
        if (_ipData.TryGetValue(ipAddress, out var ipInfo) && ipInfo.BlockedUntil.HasValue)
        {
            ...
            _ipData[ipAddress] = (ipInfo.Requests, null); // Mở khóa
        }
        ...
        if (requests.Count < _maxRequests)
        {
            ...
            _ipData[ipAddress] = (requests, ipInfo.BlockedUntil);
            return true;
        }
